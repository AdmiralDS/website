<script lang="ts">
  ...
  // import { get_current_component } from 'svelte/compiler';
  ...
  // const forwardEvents = createEventForwarder(containerElement, ['input', 'change', 'beforeinput']);
...
<div
  ...
  style="--fds-slider-percentage: {percentage}%; --fds-slider-thumb-offset: {thumbClientWidth / 2 -
    ...
    <div class="slider-tick-bar placement-{tickPlacement}" bind:this={tickBarElement}>
      {#each ticks as tick, index}
        <div
          ...
          class:alignStart={tickNameAlign === 'start'}
          class:alignEnd={tickNameAlign === 'end'}
